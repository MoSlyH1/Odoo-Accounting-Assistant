...
import { currencies, company, findPartners } from './catalog.js';
...
async function attachFile(moveId, file) {
  ...
  await call('account.move', 'message_post', [[moveId]], {
    body: 'Scanned bill uploaded by Bill Agent',
    attachment_ids: [attachmentId],
  }).catch(() => {});
  await call('account.move', 'write', [[moveId], { message_main_attachment_id: attachmentId }]).catch(() => {});
  return attachmentId;
}
...
export async function createDraft(bill) {
  ...
  try {
    attachmentId = await attachFile(moveId, bill.file);
  } catch (e) {
